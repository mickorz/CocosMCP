...
调用端点统一为 Simple API：`POST http://127.0.0.1:{{port}}/api/scene/{工具名}`，参数走 JSON body。
...
获取当前场景：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/scene/scene_management \
...
获取场景层级：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/scene/scene_hierarchy \
  -H "Content-Type: application/json" \
  -d '{}'
```
