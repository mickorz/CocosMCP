...
端点：`POST http://127.0.0.1:{{port}}/api/node/{工具名}`，参数走 JSON body。
...
查找名字含 Player 的节点：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/node/node_query \
...
创建节点：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/node/node_lifecycle \
  -H "Content-Type: application/json" \
