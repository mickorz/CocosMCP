...
端点：`POST http://127.0.0.1:{{port}}/api/referenceImage/{工具名}`，参数走 JSON body。
...
添加参考图：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/referenceImage/reference_image_management \
...
调整参考图位置：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/referenceImage/reference_image_transform \
  -H "Content-Type: application/json" \
