...
端点：`POST http://127.0.0.1:{{port}}/api/debug/{工具名}`，参数走 JSON body。
...
读取控制台最近 50 条错误日志：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/debug/debug_console \
...
搜索日志文件中的报错：
```bash
curl -X POST http://127.0.0.1:{{port}}/api/debug/debug_logs \
  -H "Content-Type: application/json" \
