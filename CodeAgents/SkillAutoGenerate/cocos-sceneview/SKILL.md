...
端点：`POST http://127.0.0.1:3001/api/sceneView/{工具名}`，参数走 JSON body。
...
切换到移动工具：
```bash
curl -X POST http://127.0.0.1:3001/api/sceneView/scene_view_gizmo_management \
...
切换 2D/3D 视图：
```bash
curl -X POST http://127.0.0.1:3001/api/sceneView/scene_view_mode_control \
  -H "Content-Type: application/json" \
