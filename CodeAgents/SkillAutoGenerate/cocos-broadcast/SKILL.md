...
端点：`POST http://127.0.0.1:3001/api/broadcast/{工具名}`，参数走 JSON body。
...
开始监听事件：
```bash
curl -X POST http://127.0.0.1:3001/api/broadcast/broadcast_listener_management \
...
获取最近 20 条广播日志：
```bash
curl -X POST http://127.0.0.1:3001/api/broadcast/broadcast_log_management \
  -H "Content-Type: application/json" \
