...
端点：`POST http://127.0.0.1:3001/api/server/{工具名}`，参数走 JSON body。
...
获取综合状态：
```bash
curl -X POST http://127.0.0.1:3001/api/server/server_information \
...
测试连通性：
```bash
curl -X POST http://127.0.0.1:3001/api/server/server_connectivity \
  -H "Content-Type: application/json" \
