...
端点：`POST http://127.0.0.1:3001/api/component/{工具名}`，参数走 JSON body。
...
列出节点上的组件：
```bash
curl -X POST http://127.0.0.1:3001/api/component/component_query \
...
给节点添加 Sprite 组件：
```bash
curl -X POST http://127.0.0.1:3001/api/component/component_manage \
  -H "Content-Type: application/json" \
