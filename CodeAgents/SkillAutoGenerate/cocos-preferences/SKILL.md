...
端点：`POST http://127.0.0.1:3001/api/preferences/{工具名}`，参数走 JSON body。
...
列出偏好分类：
```bash
curl -X POST http://127.0.0.1:3001/api/preferences/preferences_query \
...
读取 general 分类配置：
```bash
curl -X POST http://127.0.0.1:3001/api/preferences/preferences_manage \
  -H "Content-Type: application/json" \
