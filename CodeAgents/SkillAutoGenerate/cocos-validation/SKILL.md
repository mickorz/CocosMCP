...
端点：`POST http://127.0.0.1:3001/api/validation/{工具名}`，参数走 JSON body。
...
```bash
curl -X POST http://127.0.0.1:3001/api/validation/safe_string_value \
...
```bash
curl -X POST http://127.0.0.1:3001/api/validation/format_mcp_request \
  -H "Content-Type: application/json" \
