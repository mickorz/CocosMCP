...
端点：`POST http://127.0.0.1:3001/api/assetAdvanced/{工具名}`，参数走 JSON body。
...
按类型列出资源：
```bash
curl -X POST http://127.0.0.1:3001/api/assetAdvanced/asset_query \
...
刷新资源库：
```bash
curl -X POST http://127.0.0.1:3001/api/assetAdvanced/asset_system \
  -H "Content-Type: application/json" \
  -d '{"action":"refresh"}'
```
