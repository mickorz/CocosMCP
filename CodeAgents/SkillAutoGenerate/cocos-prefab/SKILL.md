...
端点：`POST http://127.0.0.1:3001/api/prefab/{工具名}`，参数走 JSON body。
...
列出预制体：
```bash
curl -X POST http://127.0.0.1:3001/api/prefab/prefab_browse \
...
实例化预制体到场景：
```bash
curl -X POST http://127.0.0.1:3001/api/prefab/prefab_instance \
  -H "Content-Type: application/json" \
